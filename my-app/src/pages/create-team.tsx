...
function CreateTeam() {
  ...
  const tokenboundAccount = (id: string) => {
    let token = tokenboundClient.createAccount({
      tokenContract: process.env
        .NEXT_PUBLIC_MANAGER_CONTRACT_ADDRESS as `0x${string}`,
      tokenId: id,
    });
  ...
  const unwatch = useContractEvent({
    ...
    listener: async (event) => {
      ...
      try {
        const token = await tokenboundAccount("1");
        console.log(token);
        alert("Tokenbound Account created");
        unwatch?.();
      } catch (e) {
        alert("Error creating tokenbound account");
        unwatch?.();
      }
    },
  });
